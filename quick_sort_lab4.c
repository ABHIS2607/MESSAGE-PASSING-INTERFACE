...
int main(int argc, char ** argv) {
  ...
  MPI_Scatter(arr, n / size, MPI_INT, localArr, n / size, MPI_INT,
  ...
  while (step < size) {
    ...
    if (rank % (2 * step) == 0) {
      if (rank + step < size) {
        int recvSize;
        MPI_Recv( & recvSize, 1, MPI_INT, rank + step, 0,

          MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        int * recvArr = (int * ) malloc(sizeof(int) * recvSize);
        MPI_Recv(recvArr, recvSize, MPI_INT, rank + step, 0,

          MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        int * mergedArr = (int * ) malloc(sizeof(int) * (localSize +
      ...
      MPI_Send(localArr, localSize, MPI_INT, sendRank, 0,
